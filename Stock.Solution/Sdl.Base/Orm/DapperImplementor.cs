...
namespace Sdl.Base.Orm
{
    public interface IDapperImplementor
    {
        ...
        //执行存储过程
        IEnumerable<T> ExecuteProc<T>(IDbConnection connection, string procName, IDbTransaction transaction, int? commandTimeout, bool buffered, params IDataParameter[] parms) where T : class;

        //DataSet ExecuteDataSetProc(IDbConnection connection, string procName, IDbTransaction transaction, int? commandTimeout, bool buffered, params IDataParameter[] parms);
    }
    ...
    public class DapperImplementor : IDapperImplementor
    {
        ...
        public IEnumerable<T> ExecuteProc<T>(IDbConnection connection, string procName, IDbTransaction transaction, int? commandTimeout, bool buffered, params IDataParameter[] parms) where T : class
        {
            DynamicParameters dynamicParameters = new DynamicParameters();
            if (parms != null)
            {
                foreach (var parameter in parms)
                {
                    ...
                    {
                        dynamicParameters.Add(parameter.ParameterName, parameter.Value, null, ParameterDirection.Output, null);
                    }
                    else
                    {
            ...
            return connection.Query<T>(procName, dynamicParameters, transaction, buffered, commandTimeout, CommandType.StoredProcedure);
        }
